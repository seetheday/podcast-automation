...
Follow PEP 8 with four-space indents, type hints, and dataclasses for immutable configs. Modules and functions use `snake_case`, classes `PascalCase`, and CLI entry points `hyphen-case` (`generate-notes`). Keep functions under 40 lines by extracting helpers. Use descriptive filenames such as `ingest_sync.py` instead of `utils.py`.
